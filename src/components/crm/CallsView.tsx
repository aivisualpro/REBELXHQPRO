...
export default function CallsView({ clientId, clientPhone, onInitiateCall }: CallsViewProps) {
  ...
  useEffect(() => {
    ...
    if (!clientId) {
      // Fetch clients for filter only if not scoped to a specific client
  ...
  const fetchActivities = useCallback(async () => {
    ...
    try {
      ...
      // If clientId prop is provided, force it. Otherwise use selected filters
      if (clientId) {
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          // Add Type=Call to imported data if missing
          const importedData = results.data.map((row: any) => ({
             ...row,
             type: row.type || 'Call',
             // If scoped to client, ensure imported data is associated (or error?) 
             // Ideally import shouldn't serve client scope unless explicitly handled.
             // We'll leave it as general import logic for now or disable import in client view.
          }));
          ...
          const loadingToast = toast.loading('Importing calls...');
          ...
          if (res.ok) {
            const data = await res.json();
            toast.success(`Imported ${data.count} calls`);
            fetchActivities();
          } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = { ...formData, type: 'Call' };
      ...
      if (res.ok) {
        ...
        toast.success(editingActivity ? 'Call updated' : 'Call logged');
        fetchActivities();
      } else {
        const err = await res.json();
    ...
    <div className="flex flex-col h-full bg-background transition-colors duration-300">
      ...
      <div className="flex items-center justify-between px-4 h-9 border-b border-border bg-secondary/50 transition-colors">
        ...
        <div className="flex items-center space-x-2">
          {/* Hide Client filter if clientId is provided */}
          {!clientId && (
              <MultiSelectFilter
                label="Client"
                icon={Briefcase}
          ...
          <button
            onClick={() => {
                ...
                openModal();
                // Additionally trigger Google Voice if phone is available
                if (onInitiateCall) {
                    onInitiateCall();
                } else if (clientPhone) {
      ...
      <div className="flex-1 overflow-x-hidden overflow-y-auto scrollbar-custom bg-background/50 relative">
        <div className="min-w-full px-2 py-2">
          ...
          <tbody className="divide-y divide-border">
            {loading ? (
              <tr><td colSpan={clientId ? 4 : 5} className="px-4 py-12 text-center text-xs text-muted-foreground italic">Loading calls...</td></tr>
            ) : error ? (
              <tr><td colSpan={clientId ? 4 : 5} className="px-4 py-12 text-center text-destructive text-xs font-bold">{error}</td></tr>
            ) : activities.length === 0 ? (
              <tr><td colSpan={clientId ? 4 : 5} className="px-4 py-12 text-center text-xs text-muted-foreground uppercase font-medium tracking-tighter opacity-50">No calls found</td></tr>
          ...
          <div className="bg-card w-full max-w-md shadow-2xl animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh] border border-border">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
              <h2 className="text-sm font-black uppercase tracking-widest text-foreground">
                {editingActivity ? 'Edit Call' : 'Log New Call'}
              </h2>
              <button
            ...
            <div className="px-6 py-4 border-t border-border bg-secondary/30 shrink-0 flex justify-end space-x-3">
              ...
              >
                ...
                <span>{editingActivity ? 'Save Changes' : 'Log Call'}</span>
              </button>
            </div>
