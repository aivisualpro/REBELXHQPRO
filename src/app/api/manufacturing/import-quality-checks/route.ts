...
export async function POST(request: Request) {
    try {
        ...
        // Helper to parse boolean from CSV (handles "TRUE", "true", "1", "yes", etc.)
        ...
        for (const [woRef, rows] of groups.entries()) {
            ...
            const newQcEntries: any[] = [];
            for (const row of rows) {
                newQcEntries.push({
                    _id: new mongoose.Types.ObjectId(),
                    checkedBy: cleanStr(row.checkedBy),
                    packagedBy: cleanStr(row.packagedBy),
                    label: parseBool(row.label),
                    lot: parseBool(row.lot),
                    seal: parseBool(row.seal),
                    packageQuality: parseBool(row.packageQuality),
                    repackaged: parseBool(row.repackaged),
                    weight: parseNum(row.weight) ?? 0,
                    target: parseNum(row.target) ?? 0,
                    actualWeight: parseNum(row.actualWeight) ?? 0,
                    qualityCheckedBy: cleanStr(row.qualityCheckedBy),
                    createdAt: parseDate(row.createdAt)
                });
            }

            if (newQcEntries.length > 0) {
                operations.push({
                    updateOne: {
                        filter: { _id: new mongoose.Types.ObjectId(mfgId) },
                        update: { $push: { qualityCheck: { $each: newQcEntries } } }
                    }
                });
            }
        }
        ...
        if (operations.length > 0) {
            try {
                ...
                operations.forEach(op => count += op.updateOne.update.$push.qualityCheck.$each.length);
            } catch (bulkErr: any) {
        ...
        console.error('Manufacturing Quality Check Import Error:', error);
