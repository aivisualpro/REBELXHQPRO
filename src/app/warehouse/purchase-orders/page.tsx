...
import {
  Search, ArrowUpDown, Plus, Trash2, X, Loader2, Calendar, ChevronDown
} from 'lucide-react';
...
interface CacheEntry {
  ...
  datePreset: string; fromDate: string; toDate: string;
...
function PurchaseOrdersContent() {
  ...
  useEffect(() => {
    ...
    // (We skip vendors as it's an array and might need more complex state syncing, unless requested)
      ...
      if (res.ok) {
        ...
        if (isAppend) {
          setOrders(prev => {
            ...
            globalCache.current = { orders: merged, hasMore: newHasMore, page: pageNum, total: newTotal, sortBy, sortOrder, search: debouncedSearch, statuses: [activeStatus], vendors: selectedVendors, datePreset, fromDate, toDate, timestamp: Date.now() };
            return merged;
          });
        } else {
          setOrders(newOrders);
          setTotal(newTotal);
          globalCache.current = { orders: newOrders, hasMore: newHasMore, page: pageNum, total: newTotal, sortBy, sortOrder, search: debouncedSearch, statuses: [activeStatus], vendors: selectedVendors, datePreset, fromDate, toDate, timestamp: Date.now() };
        }
  ...
  }, [sortBy, sortOrder, debouncedSearch, activeStatus, selectedVendors, fromDate, toDate]);
  ...
  useEffect(() => {
    if (isFirstMount.current) {
      ...
      if (cache && cache.orders.length > 0 && (Date.now() - cache.timestamp) < CACHE_TTL &&
        ...
        JSON.stringify(cache.statuses) === JSON.stringify([activeStatus]) && cache.datePreset === datePreset && cache.fromDate === fromDate && cache.toDate === toDate) {
  ...
  }, [sortBy, sortOrder, debouncedSearch, activeStatus, selectedVendors, datePreset, fromDate, toDate]);
