...
export default function CallsPage() {
  ...
  }, [page, debouncedSearch, sortBy, sortOrder, selectedClients, selectedCreatedBy]);
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          // Add Type=Call to imported data if missing, or validate?
          // For now, assume generic import or just pass it through. 
          // The backend might default type? 
          // Let's modify data to ensure type is Call if we want to enforce it.
    ...
    <div className="flex flex-col h-[calc(100vh-48px)] bg-white">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-100 bg-slate-50/50">
        <div className="flex items-center space-x-4">
          <h1 className="text-sm font-bold text-slate-900 uppercase tracking-tighter flex items-center gap-2">
        ...
        <div className="flex items-center space-x-2">
          {/* Removed Type Filter because this is strictly Calls */}
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full border-collapse text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              <tr><td colSpan={5} className="px-4 py-12 text-center text-xs text-slate-400">Loading calls...</td></tr>
            ) : error ? (
              <tr><td colSpan={5} className="px-4 py-12 text-center text-red-500 text-xs font-bold">{error}</td></tr>
            ) : activities.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-12 text-center text-xs text-slate-400 uppercase font-bold tracking-tighter opacity-50">No calls found</td></tr>
